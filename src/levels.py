...
class Level:
    ...
    Atributos:
    window -- La ventana donde se dibuja el juego.
    player_name -- Nombre del jugador.
    max_score -- Puntaje máximo necesario para ganar el nivel.
    level_number -- Número del nivel actual.
    pause_menu -- Función que maneja el menú de pausa.
    game_over_menu -- Función que maneja el menú de fin de juego.
    enemies -- Grupo de enemigos en el nivel.
    objects -- Grupo de objetos en el nivel.
    floor -- Superficie que representa el suelo.
    background_image -- Imagen de fondo del nivel.
    clock -- Reloj para controlar el FPS.
    start_ticks -- Tiempo en que comenzó el nivel.
    pause_start -- Tiempo en que se inició la pausa.
    total_pause_time -- Tiempo total acumulado en pausas.
    score -- Puntaje actual del jugador.
    background -- Fondo del nivel.
    bg_image -- Imagen del fondo.
    block_size -- Tamaño del bloque en el nivel.
    player -- Jugador en el nivel.
    coins -- Grupo de monedas en el nivel.
    running -- Estado del juego (activo o no).
    game_win_menu -- Función que maneja el menú de victoria.

    Métodos:
    __init__(self, window, pause_menu, game_over_menu, enemies, objects, floor, background_image, max_score, level_number, player_name, game_win_menu) -- Inicializa el nivel con sus parámetros.
    run(self) -- Ejecuta el bucle principal del nivel.
    """
    ...
    def run(self):
        ...
        while self.running:
            ...
            if self.score == 6 and not enemies_added:
                ...
                new_enemy1 = Enemy(330, 317, 50, 50, ("Characters", "Enemy", 48, 48, True))
                new_enemy2 = Enemy(580, 510, 50, 50, ("Characters", "Enemy2", 48, 48, True))
                new_enemy3 = Enemy(140, 510, 50, 50, ("Characters", "Enemy", 48, 48, True))
                new_enemy4 = Enemy(70, 517, 50, 50, ("Characters", "Enemy2", 48, 48, True))
...
def pause_menu(window):
    ...
    background_pause = pygame.transform.scale(pygame.image.load("./src/assets/Background/pausa.jpg"), size_screen)
...
def game_over_menu(window):
    ...
    background_menu = pygame.transform.scale(pygame.image.load("./src/assets/Background/game over.jpeg"), (WIDTH, HEIGHT))
...
def game_win_menu(window):
    ...
    window -- La ventana donde se dibuja el menú de victoria.
    """
    from menus import main_menu
    ...
    buttons = [back_button, quit_button]
    background_menu = pygame.transform.scale(pygame.image.load("./src/assets/Background/victoria.jpeg"), (WIDTH, HEIGHT))
